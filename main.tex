...
% ============================================================
  ...
  \textbf{Static minsup mode (UGenMax, UFPMax):} Vary minsup across appropriate ranges per dataset.

  \textbf{Top-K mode (UGenMax, UFPMax):} Vary $K \in \{5, 10, 20, 50, 100\}$ on mushroom\_medium, retail\_medium, and accidents\_medium.
  ...
  PerformanceTracker uses \texttt{System.nanoTime()} for timing and \texttt{Runtime.totalMemory() - Runtime.freeMemory()} for memory, with GC forced before each run. The expected-support sweep and the Top-K sweep are each averaged over 3 measured runs after 1 warm-up run. The cross-dataset scalability snapshot is a single run, which is why the numbers in Table~\ref{tab:scalability} differ slightly from the corresponding rows of the averaged tables.
...
% ============================================================
  ...
  \begin{table}[h]
    ...
    \begin{tabular}{rrrrrr}
      ...
      \midrule
      1000 & 18 & 151 & 102 & 96.0 & 67.0 \\
      2000 & 6 & 91 & 73 & 104.0 & 64.0 \\
      3000 & 4 & 89 & 67 & 100.0 & 62.0 \\
      5000 & 4 & 89 & 69 & 100.0 & 62.0 \\
      8000 & 2 & 77 & 56 & 91.0 & 59.0 \\
      10000 & 2 & 73 & 61 & 91.0 & 59.0 \\
      \bottomrule
    \end{tabular}
  \end{table}

  \textbf{Observations on retail\_medium.} As minsup rises, both the MFI count and execution time decrease, consistent with anti-monotonicity pruning. UGenMax is faster than UFPMax at every minsup tested, with speedups ranging from 16\% at minsup=10000 (61 vs.\ 73 ms) up to 33\% at minsup=1000 (102 vs.\ 151 ms), and it uses roughly 30--38\% less memory because the weighted-tidset representation is more compact than UFPMax's projected conditional databases on this sparse dataset. Absolute runtime stays well under 200 ms even at the most permissive threshold, because most of the 16,470 items are infrequent and pruned early --- the cost is dominated by database loading and tidset/projection construction rather than by the search itself. At high minsup ($\geq 3000$) only 2--4 MFIs are found, which is typical for a sparse transactional dataset where long co-occurrence patterns are rare.
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot[color=blue, mark=square*] coordinates {
          (1000,151) (2000,91) (3000,89) (5000,89) (8000,77) (10000,73)
        };
        \addplot[color=red, mark=triangle*] coordinates {
          (1000,102) (2000,73) (3000,67) (5000,69) (8000,56) (10000,61)
        };
  ...
  \begin{table}[h]
    ...
    \begin{tabular}{rrrrrr}
      ...
      & & UFPMax & UGenMax & UFPMax & UGenMax \\
      \midrule
      500 & 314 & 223 & 137 & 77.3 & 48.6 \\
      1000 & 73 & 88 & 47 & 16.8 & 33.5 \\
      1500 & 30 & 51 & 32 & 28.9 & 12.4 \\
      2000 & 14 & 28 & 15 & 9.1 & 19.0 \\
      2500 & 9 & 21 & 12 & 27.4 & 12.5 \\
      3000 & 5 & 12 & 12 & 18.7 & 9.4 \\
      \bottomrule
    \end{tabular}
  \end{table}

  \textbf{Observations on mushroom\_medium:} The same qualitative trend holds --- UGenMax is faster than UFPMax at every minsup with a non-trivial workload (37--47\% speedup at minsup~$\leq$~2500), with the two algorithms tying at minsup=3000 where the work is so small (5 MFIs, 12 ms) that constant-time overhead dominates. Memory use is also generally lower or comparable. Because mushroom is much denser (120 items, $\sim$23 items per transaction), it produces far more MFIs at comparable relative thresholds (up to 314 MFIs at minsup=500), giving a larger absolute speedup window than the sparser retail dataset.
  ...
  \begin{figure}[h]
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot[color=blue, mark=square*] coordinates {
          (500,223) (1000,88) (1500,51) (2000,28) (2500,21) (3000,12)
        };
        \addplot[color=red, mark=triangle*] coordinates {
          (500,137) (1000,47) (1500,32) (2000,15) (2500,12) (3000,12)
        };
        \legend{UFPMax, UGenMax}
      \end{axis}
    \end{tikzpicture}
  ...
  \begin{table}[h]
    ...
    \begin{tabular}{rrrrrr}
      ...
      & & UFPMax & UGenMax & UFPMax & UGenMax \\
      \midrule
      20,000 & 1,285 & 126.4 & 109.1 & 761.2 & 1,103.8 \\
      30,000 & 453 & 61.5 & 46.5 & 1,190.6 & 782.4 \\
      40,000 & 236 & 38.3 & 24.8 & 912.5 & 1,041.6 \\
      50,000 & 154 & 24.7 & 14.8 & 717.2 & 1,511.8 \\
      60,000 & 87 & 16.5 & 9.4 & 937.3 & 1,158.6 \\
      70,000 & 48 & 11.7 & 6.0 & 899.3 & 1,332.1 \\
      \bottomrule
    \end{tabular}
  \end{table}

  \textbf{Observations.} Both algorithms scale to 340K transactions, returning results in tens of seconds to a couple of minutes. UGenMax is consistently faster than UFPMax across the entire minsup range on this very dense dataset, with speedups growing from 14\% at minsup=20,000 (109.1 s vs.\ 126.4 s) up to 49\% at minsup=70,000 (6.0 s vs.\ 11.7 s) as the result set shrinks and progressive focusing becomes more effective. That the search at minsup=20,000 still completes in roughly two minutes while producing 1,285 MFIs shows that both algorithms handle large result sets effectively. Memory usage varies between $\sim$700 MB and $\sim$1.5 GB due to the large number of items per transaction ($\sim$33 items on average out of 468); on this very dense dataset UGenMax's weighted tidsets are not always smaller than UFPMax's projected databases --- UFPMax uses 717--1,191 MB and UGenMax uses 782--1,512 MB, a reversal of the memory advantage observed on sparse retail. This is consistent with the fact that weighted tidsets become large when most transactions contain most items.
  ...
  \begin{figure}[h]
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot[color=blue, mark=square*] coordinates {
          (20000,126.4) (30000,61.5) (40000,38.3) (50000,24.7) (60000,16.5) (70000,11.7)
        };
        \addplot[color=red, mark=triangle*] coordinates {
          (20000,109.1) (30000,46.5) (40000,24.8) (50000,14.8) (60000,9.4) (70000,6.0)
        };
        \legend{UFPMax, UGenMax}
      \end{axis}
    \end{tikzpicture}
  ...
  \begin{table}[h]
    ...
    \begin{tabular}{lrrrrrrr}
      ...
      \midrule
      mushroom & 8,416 & 120 & 2,000 & 14 & UFPMax & 69 & 20.8 \\
      mushroom & 8,416 & 120 & 2,000 & 14 & UGenMax & 31 & 19.1 \\
      retail & 88,162 & 16,470 & 5,000 & 4 & UFPMax & 217 & 100.0 \\
      retail & 88,162 & 16,470 & 5,000 & 4 & UGenMax & 88 & 62.0 \\
      accidents & 340,183 & 468 & 50,000 & 154 & UFPMax & 26,673 & 501.6 \\
      accidents & 340,183 & 468 & 50,000 & 154 & UGenMax & 15,239 & 1,001.4 \\
      \bottomrule
    \end{tabular}
  \end{table}

  \textbf{Observation:} Both algorithms scale from 8K to 340K transactions, and UGenMax retains a clear time advantage at every scale (55\% faster on mushroom, 59\% faster on retail, 43\% faster on accidents in this snapshot). The retail run in particular illustrates UGenMax's memory advantage on sparse data ($\sim$38\% less memory than UFPMax), while on very dense accidents the weighted-tidset representation becomes larger than UFPMax's projected databases ($\sim$1,001 MB vs.\ 502 MB) --- a clear time/memory trade-off that depends on dataset density.
...
Tables~\ref{tab:topk_mushroom}, \ref{tab:topk_retail}, and~\ref{tab:topk_accidents} compare UGenMax and UFPMax in Top-K mode on mushroom\_medium, retail\_medium, and accidents\_medium for $K \in \{5,10,20,50,100\}$. Both algorithms support Top-K mode.

\begin{table}[h]
\centering
\caption{Top-K mode on mushroom\_medium (8,416 transactions, 120 items)}
\label{tab:topk_mushroom}
\begin{tabular}{rrrrrrr}
\toprule
\textbf{K} & \multicolumn{2}{c}{\textbf{Time (ms)}} & \multicolumn{2}{c}{\textbf{Memory (MB)}} & \multicolumn{2}{c}{\textbf{Final minsup}} \\
 & UFPMax & UGenMax & UFPMax & UGenMax & UFPMax & UGenMax \\
\midrule
5 & 908 & 162 & 33.9 & 21.7 & 1,711.2 & 2,451.6 \\
10 & 954 & 146 & 48.3 & 29.5 & 1,225.8 & 1,975.7 \\
20 & 963 & 176 & 51.0 & 32.2 & 973.7 & 1,332.9 \\
50 & 1,178 & 298 & 20.8 & 18.7 & 613.9 & 955.1 \\
100 & 1,323 & 443 & 40.5 & 37.3 & 477.5 & 720.8 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Top-K mode on retail\_medium (88,162 transactions, 16,470 items)}
\label{tab:topk_retail}
\begin{tabular}{rrrrrrr}
\toprule
\textbf{K} & \multicolumn{2}{c}{\textbf{Time (ms)}} & \multicolumn{2}{c}{\textbf{Memory (MB)}} & \multicolumn{2}{c}{\textbf{Final minsup}} \\
 & UFPMax & UGenMax & UFPMax & UGenMax & UFPMax & UGenMax \\
\midrule
5 & 16,604 & 594 & 203.9 & 105.0 & 936.8 & 2,248.1 \\
10 & 17,116 & 560 & 123.4 & 102.0 & 1,124.0 & 1,630.4 \\
20 & 17,315 & 675 & 66.3 & 108.0 & 646.8 & 741.7 \\
50 & 18,135 & 1,531 & 106.8 & 131.0 & 320.2 & 519.8 \\
100 & 23,936 & 4,755 & 149.2 & 101.1 & 123.7 & 260.6 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Top-K mode on accidents\_medium (340,183 transactions, 468 items). Times in seconds.}
\label{tab:topk_accidents}
\begin{tabular}{rrrrrrr}
\toprule
\textbf{K} & \multicolumn{2}{c}{\textbf{Time (s)}} & \multicolumn{2}{c}{\textbf{Memory (MB)}} & \multicolumn{2}{c}{\textbf{Final minsup}} \\
 & UFPMax & UGenMax & UFPMax & UGenMax & UFPMax & UGenMax \\
\midrule
5 & 91.7 & 10.4 & 870.4 & 1,351.9 & 79,627 & 137,418 \\
10 & 93.5 & 12.1 & 837.3 & 829.8 & 68,709 & 108,265 \\
20 & 106.0 & 14.8 & 862.7 & 1,382.9 & 54,132 & 75,922 \\
50 & 133.6 & 12.4 & 818.7 & 1,346.7 & 35,942 & 60,157 \\
100 & 130.3 & 18.0 & 1,130.5 & 1,187.2 & 30,162 & 47,455 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Key findings:}
\begin{itemize}
  \item UGenMax is dramatically faster than UFPMax in Top-K mode across all three datasets and all values of $K$: roughly \textbf{3--7$\times$ faster on mushroom}, \textbf{5--30$\times$ faster on retail}, and \textbf{7--11$\times$ faster on accidents}. The advantage is largest on retail (e.g.\ 0.6 s vs.\ 16.6 s at $K{=}5$) because UFPMax must build expensive projections over 16K items while UGenMax's dynamic threshold raising prunes most of them upfront.
  \item Dynamic threshold raising (Pruning~3) is what drives this gap: UGenMax's final minsup is consistently \emph{higher} than UFPMax's at the same $K$ (e.g.\ on mushroom at $K{=}10$: 1,976 vs.\ 1,226), which means UGenMax aggressively shrinks the search space during traversal.
  \item As $K$ increases, both algorithms take longer because the threshold stabilises at a lower value, expanding the search space (e.g.\ on retail UGenMax goes from 0.6 s at $K{=}5$ to 4.8 s at $K{=}100$).
  \item Memory behaviour mirrors the static-minsup results: UGenMax uses less memory than UFPMax on the sparse retail and the dense mushroom in most cases, but uses noticeably more on very dense accidents, again reflecting the cost of weighted tidsets when most transactions contain most items.
\end{itemize}

  \section{Summary of Findings}

  Taking the Chapter~6 results together, three findings stand out. First, correctness holds throughout: UGenMax and UFPMax produce identical MFI sets across all datasets and thresholds. Second, on the primary retail\_medium dataset UGenMax is 16--33\% faster than UFPMax and uses roughly 30--38\% less memory --- a time advantage that generalises to dense mushroom (37--47\% faster on non-trivial workloads) and to very dense accidents (14--49\% faster across the full minsup range), although on accidents the weighted-tidset representation becomes larger than UFPMax's projected databases, so memory savings do not generalise to the very-dense regime. Finally, both algorithms scale to 340K transactions (accidents) within acceptable time limits, and UGenMax additionally outperforms UFPMax in Top-K mode by a substantial margin (3--7$\times$ on mushroom, 5--30$\times$ on retail, 7--11$\times$ on accidents) thanks to dynamic threshold raising.
...
% ============================================================
  ...
  Several limitations remain. Maximality checking uses a linear scan ($O(|\text{MFI}|)$ per check), which a trie- or hash-based index could improve. Memory usage on the accidents dataset also reaches roughly 1.5\,GB on the densest configurations, which may be restrictive in memory-constrained environments; this is most pronounced for UGenMax, whose weighted-tidset representation grows large when most transactions contain most items.
