...
\end{algorithmic}
  ...
  APFI-MAX uses tighter pruning via expectation bounds derived from the CLT. A lower bound $lb(E)$, obtained from the Chernoff inequality, rules out any itemset with $E[\text{sup}(X)] < lb(E)$, which cannot be a p-FP regardless of its variance. An upper bound $ub(E) = T + z_\tau \cdot \sqrt{E}$, with $z_\tau = \Phi^{-1}(1 - \tau)$, certifies any itemset with $E[\text{sup}(X)] \geq ub(E)$ as definitely a p-FP without needing a full PMF computation.
...
% ============================================================
  ...
  Correctness is validated in three ways. All four algorithms are first hand-verified on the small contextUncertain example. UGenMax and UFPMax are then cross-checked against each other on every configuration to confirm they produce identical MFI sets. TODIS-MAX and APFI-MAX are similarly compared; although APFI-MAX may in principle produce slightly different results because of the CLT approximation, it should closely match TODIS-MAX on the datasets we evaluate.
...
% ============================================================
  ...
  \textbf{Observations on retail\_medium.} Across all 15 configurations, TODIS-MAX and APFI-MAX return the same MFI count, so the CLT approximation preserves correctness on this 88K-transaction dataset. Most runs take 60--66\,s for both algorithms, because the total time is dominated by database loading and Apriori candidate generation across the 16,470-item space, not by the frequency-measurement step; the $O(n)$ versus $O(n \cdot T)$ per-candidate difference has little effect here because only 0--4 candidates survive Apriori. A few outliers stand out --- TODIS-MAX at (minsup=5000, $\tau$=0.7) took 135.3\,s, and APFI-MAX took 90.8\,s at (5000, 0.9) and 118.1\,s at (10000, 0.5) --- but these reflect the non-deterministic portion of the Apriori phase (hashing and GC effects) rather than the algorithmic core. The clearest separation between the two algorithms is in memory: APFI-MAX sits at a steady 25--53\,MB across all non-trivial configurations, while TODIS-MAX spikes to 127--449\,MB whenever its DP array must be allocated. When no MFIs are found (minsup=30,000), the two algorithms converge to 21.1\,MB each.
  ...
  \textbf{Observations on mushroom\_medium.} MFI counts match across all 15 rows, again confirming that the CLT approximation matches the exact DP result. APFI-MAX is 12--17$\times$ faster whenever there are surviving candidates to verify (minsup $\in \{3000, 4000\}$), for example 519/37\,$\approx$\,14.0$\times$, 488/30\,$\approx$\,16.3$\times$, and 304/23\,$\approx$\,13.2$\times$; for minsup $\geq 5000$ the Apriori phase prunes everything before frequency measurement, so both algorithms finish in 16--26\,ms and the speedup vanishes. Memory is also consistently lower for APFI-MAX when any PMF computation is actually performed (6--35\,MB vs.\ up to 160\,MB for TODIS-MAX), because APFI-MAX stores only running sums instead of a length-$T$ DP array.
  ...
  \begin{figure}[h]
    ...
    \begin{tikzpicture}
      ...
      ybar,
      bar width=12pt,
      xlabel={Configuration (minsup, $\tau$)},
      ylabel={Execution Time (ms)},
      legend pos=north west,
      legend style={font=\small},
      grid=major,
      grid style={dashed, gray!30},
      ymin=0,
      symbolic x coords={3000/0.5, 3000/0.7, 3000/0.9, 4000/0.5, 4000/0.7, 4000/0.9},
      xtick=data,
      xticklabel style={font=\footnotesize, rotate=30, anchor=east},
      yticklabel style={font=\small},
      nodes near coords,
      nodes near coords style={font=\tiny},
      every node near coord/.append style={yshift=2pt},
        ...
        \legend{TODIS-MAX, APFI-MAX}
      \end{axis}
    \end{tikzpicture}
    \caption{TODIS-MAX vs.\ APFI-MAX on mushroom\_medium (12--17$\times$ speedup for APFI-MAX)}
    \label{fig:prob_mushroom_time}
  \end{figure}
...
Table~\ref{tab:topk} compares UGenMax and UFPMax in Top-K mode on mushroom\_medium. Top-K is only supported by the expected support algorithms; TODIS-MAX and APFI-MAX require explicit minsup and minprob thresholds.
...
\end{itemize}
  ...
  The expected support model (UFPMax/UGenMax) and the probabilistic support model (TODIS-MAX/APFI-MAX) serve complementary purposes. Expected support is faster and simpler but treats all transactions equally: an itemset with 0.01 probability in each of 1000 transactions can have the same expected support as one with 1.0 probability in 10 transactions. Probabilistic support distinguishes these cases by asking ``what is the probability that this itemset appears at least $T$ times?'', giving a more statistically rigorous measure of frequency. For practitioners, then, expected support is best suited to quick exploratory analysis, while probabilistic support should be preferred when statistical guarantees matter.

  \section{Summary of Findings}

  Taking the Chapter~6 results together, four findings stand out. First, correctness holds throughout: UGenMax and UFPMax produce identical MFI sets across all datasets and thresholds, and TODIS-MAX and APFI-MAX produce matching MFI counts across every configuration we measured. Second, on the primary retail\_medium dataset UGenMax is 19--43\% faster than UFPMax and uses roughly 30--35\% less memory --- an advantage that generalises to dense mushroom (42--66\% faster) and to very dense accidents (13--36\% faster at minsup $\geq$ 30,000, essentially tied at minsup=20,000). Third, on retail\_medium the two probabilistic algorithms have comparable total time --- because Apriori candidate generation dominates --- but APFI-MAX uses dramatically less memory (25--53\,MB vs.\ up to 449\,MB for TODIS-MAX), and on the smaller mushroom dataset APFI-MAX is additionally 12--17$\times$ faster whenever there are surviving candidates. Finally, all algorithms scale to 340K transactions (accidents) within acceptable time limits, and UGenMax retains its per-dataset speed advantage across the whole range from 8K to 340K transactions.
...
% ============================================================
  ...
  This project designed, implemented, and evaluated four algorithms for mining frequent maximal itemsets from uncertain transaction databases, covering two support models. Under the expected support model, UGenMax (bottom-up weighted tidset intersections, adapted from GenMax) and UFPMax (top-down conditional database projection, adapted from FPMax) both use branch-and-bound with three pruning strategies: anti-monotonicity, progressive focusing, and dynamic Top-K threshold raising. Under the probabilistic support model, TODIS-MAX (exact PMF computation via Dynamic Programming, based on Sun et al., 2010) and APFI-MAX (approximate frequency estimation via CLT, based on Chen et al., 2020) share an Apriori-based candidate generation phase followed by top-down maximal verification.
  ...
  The project's main contributions are as follows. We formalised the problem with precise mathematical definitions for both the expected support and the probabilistic support models. We adapted GenMax and FPMax for uncertain data with expected support, including a Top-K mode, and implemented TODIS-MAX and APFI-MAX for probabilistic maximal frequent itemset mining. The whole code base is pure Java SE with no external dependencies, organised into algorithm classes (no \texttt{main}) and experiment classes (with \texttt{main} and statistics output). Finally, we carried out a comprehensive experimental comparison across three datasets, four algorithms, and varying parameter configurations at medium uncertainty, showing that UGenMax consistently outperforms UFPMax in speed, and that APFI-MAX matches TODIS-MAX in correctness while using dramatically less memory and, on mushroom, running an order of magnitude faster.

  \section{Limitations}

  Several limitations remain. Maximality checking uses a linear scan ($O(|\text{MFI}|)$ per check), which a trie- or hash-based index could improve. APFI-MAX's CLT approximation may become less accurate for small datasets where the normal approximation is poor, and neither TODIS-MAX nor APFI-MAX currently supports Top-K mode. Memory usage on the accidents dataset also reaches roughly 1.4\,GB at low thresholds, which may be restrictive in memory-constrained environments.

  \section{Future Work}

  A natural next step is a diffset optimisation for UGenMax to reduce memory usage, followed by parallel search across independent subtrees for multi-core speedup. The framework could also be extended to mining closed or sequential patterns from uncertain data, and evaluated on real-world uncertain datasets from sensor networks or noisy data integration. Adding a Top-K mode to the probabilistic algorithms, and exploring GPU acceleration for the DP-based PMF computation in TODIS-MAX, are further promising directions.
